...
const SLACK_WEBHOOK_URL = "https://hooks.slack.com/services/xxxxxxxxx/xxxxxxxxx/xxxxxxxxxxxxx";
// e.g., '1AbCdEfGh123...'
const SPREADSHEET_ID = 'xxxxxxxxxxx';
const SHEET_NAME = '更新管理';
  ...
  if (!rowCountChanged && !colCountChanged) {
    for (let row = 0; row < values.length; row++) {
      for (let col = 0; col < values[0].length; col++) {
        ...
        const oldValue = (lastKnownData[row] && typeof lastKnownData[row][col] !== 'undefined') ? lastKnownData[row][col] : undefined;
        ...
        if (currentNormalized !== oldNormalized) {
          ...
          for (let c = 0; c < sendText.length; c++) {
            changedCells.push(`${row + 1}行目・${col + 1}列目の、CPID：${values[row][0]}の更新内容\nAction：${sendText[c].action}\n属性：${sendText[c].attributes.join(",")}\n変更内容：${sendText[c].textContent}\n------------------------------------------------------------------`);
          }
        }
      }
    }
